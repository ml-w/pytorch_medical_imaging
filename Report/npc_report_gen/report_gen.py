...
class ReportGen_NPC_Screening(Canvas):
    ...
    def enrich_mri_frame(self):
        ...
        try:
            im, sn = self.draw_image(data_display['image_nii'], data_display['segment_nii'], return_num_seg=True,
                                     risk_data=data_display['risk_data'])
        except:
        ...
        if self.diagnosis_overall:
            try:
                im = self.draw_image(data_display['image_nii'], data_display['segment_nii'], return_num_seg=False,
                                     mode=2, risk_data=data_display['risk_data'])
            except:
                self._logger.error("Something went wwrong when drawing the second page.")
                im = self.draw_image(data_display['image_nii'], data_display['segment_nii'], return_num_seg=False,
                                     mode=3, risk_data=data_display['risk_data'])
        else:
    ...
    def draw_image(self, img, seg,  mode=None, return_num_seg=False, risk_data=None):
        ...
        The risk data were generated by rAIdiologist.
        """
    ...
    def crop_image(image : torch.Tensor,
        ...
        if len(center) > 2 or len(size) > 2:
            msg = f"Center and size should be an iterable with two elements."
            raise ArithmeticError(msg)
        if not isinstance(image, torch.Tensor):
            msg = f"Image should be a torch Tensor, but got {type(image)}."
            raise TypeError(msg)

        lower_bound = [np.max([0, int(c - s // 2)]) for c, s in zip(center, size)]
        upper_bound = [np.min([l + s, m]) for l, s, m in zip(lower_bound, size, image.shape[1:])]
