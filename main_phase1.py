...
def main(a):
    ...
    if not mode:
        ...
        writer = SummaryWriter('/media/storage/PytorchRuns/MCA')
            ...
            for index, samples in enumerate(loader):
                ...
                if a.plot:
                    try:
                        ...
                        writer.add_image('Image/Image', poolim, index + i * len(loader))
                        writer.add_image('Image/Groundtruth', poolgt, index + i * len(loader))
                        writer.add_scalar('Loss', loss.data[0], index + i * len(loader))
                        # del poolim, poolgt
        ...
        for i, samples in enumerate(tqdm(loader)):
            try:
                ...
                if a.usecuda:
                    s = s.cuda()
                out = net.forward(s.unsqueeze(1))
                out = ImageDataSet.WrapImageWithMetaData(out.data.cpu().squeeze().numpy(), inputDataset.metadata[i])

                outname = a.output + "/" + os.path.basename(inputDataset.dataSourcePath[i])
                tqdm.write("Writing " + outname)
                WriteImage(out, outname)
            except:
                tqdm.write("Error occured for sample " + inputDataset.dataSourcePath[i])
        pass
