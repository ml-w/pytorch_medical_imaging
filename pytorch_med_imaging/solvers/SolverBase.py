...
class SolverBase(object):
    ...
    Key Methods:
        - :meth:`step()` - This method aims to process one mini-batch.
    ...
    def step(self, *args) -> Tuple[Any, float]:
        ...
        If you don't want to inherit the class but still need some control
        ...
        .. hint::
            If you do not wish to override the class but wants some flexibility of I/O, you can implement a static
            method with signature `step_net(net, *args, loss)` in network. The solver will use this static method
            as step forward function. The `step_net` function is expected to return both network prediction and the
            loss calculated.

        Returns:
            Any: Output generated by model
            float: Loss value of the output

        """
        if not hasattr(self.get_net(), 'step_net'):
            out = self._feed_forward(*args)
            loss = self._loss_eval(out, *args) # *args = (s, g)
        else:
            # if step_net static method exist in network, use it
            func_step_train = self.get_net().step_net
            out, loss = func_step_train(self.net, *args, self._loss_eval)
    ...
    def solve_epoch(self, epoch_number):
        ...
        for step_idx, mb in enumerate(data_loader):
            ...
            if self.max_step >= 1:
                if not step_idx < int(self.max_step):
                    break
            ...
            self._step_callback(self._var_to_cpu(s), self._var_to_cpu(g), out, self._var_to_cpu(loss.data),
    ...
    @staticmethod
    def _var_to_cpu(var: Union[Tuple[torch.Tensor], torch.Tensor]) -> Union[torch.Tensor, List[torch.Tensor]]:
        r"""Move variable to tensor"""
        if isinstance(var, (tuple, list)):
            return [SolverBase._var_to_cpu(v) for v in var]
