"""
This code is copied from https://github.com/junyuchen245/ViT-V-Net_for_3D_Image_Registration_Pytorch

Several modifications were implemented to make this work stand-alone
...
def get_3DReg_config():
    config = ViT_Config()
    config.patches = ViT_Config()
    config.patches.size = (32, 32, 1)
    # config.patches.grid = (16, 16, 10)
    config.hidden_size = 252
    config.transformer = ViT_Config()
    ...
    config.encoder_channels = (32, 32, 1)
    ...
    return config
default_config = get_3DReg_config()
...
class Embeddings(nn.Module):
    ...
    def __init__(self, config, img_size):
        ...
        self.hybrid_model = CNNEncoder(config, n_channels=config.in_channels)
...
class ViTVNet(nn.Module):
    def __init__(self, config=default_config, img_size=(64, 256, 256), int_steps=7, vis=False):
