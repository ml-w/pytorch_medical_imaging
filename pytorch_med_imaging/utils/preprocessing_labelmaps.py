...
def remap_label(map_dict: dict,
    ...
    dataloader = DataLoader(data, batch_size=1, num_workers=0)

    out_tensor =[]
    for i, dat in enumerate(auto.tqdm(dataloader)):
        out_tensor.append(dat['seg'][tio.DATA].permute(0, 1, 4, 3, 2)) # (B x C x W x H x D) to (B x C x D x W x H)

    inputdata.write_all(out_tensor, output_dir)
