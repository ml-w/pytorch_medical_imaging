...
from MedImgDataset import ImageDataSet2D, ImageFeaturePair, Landmarks, Projection_v2
...
def main(a):
    ...
    # Training Mode
    if not mode:
        inputDataset= Projection_v2(a.input, dtype=np.float32, verbose=True, cachesize=4)
        gtDataset   = Projection_v2(a.train, dtype=np.float32, verbose=True, cachesize=4)
        ...
        for i in range(a.epoch):
            ...
            for index, samples in enumerate(tqdm(loader, desc="Steps")):
                if a.usecuda:
                    s = Variable(samples[0][0]).float().cuda()
                    g = Variable(samples[1][0]).float().cuda()
                    f = Variable(samples[0][1]).float().cuda()
                else:
                    s, g, f = Variable(samples[0][0]), Variable(samples[1][0]), Variable(samples[0][1])

                out = net.forward(s.unsqueeze(1), f)
                ...
                if a.plot:
                    visualization.Visualize2D(s.squeeze().cpu().permute(0, 2, 1).data,
                                              ...
                                              env="CT_SinoFilter_ResNetB", indexrange=[0,25], nrow=1)
